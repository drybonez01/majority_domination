...
if len(sys.argv) > 1:
    ...
    weights_file_path = weightDef(graph_file_path, working_dir)

    seedset_file_path = None
    while True:
        alg = int(input("\nInserire l'algoritmo da utilizzare per la selezione del seed set (1,2,3): "))
        if alg == 1:
            seedset_file_path = seedset_alg1(graph_file_path, weights_file_path, working_dir)
            break
        elif alg == 2:
            seedset_file_path = seedset_alg2(graph_file_path, weights_file_path, working_dir)
            break
        elif alg == 3:
            seedset_file_path = seedset_alg3(graph_file_path, weights_file_path, working_dir)
            break
        else:
            print("Opzione non valida. Riprovare.")

    majority_influence_diffusion(graph_file_path, seedset_file_path, working_dir)
